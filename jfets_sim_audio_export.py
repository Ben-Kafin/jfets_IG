    ...
    def solve_transient(self, input_node, monitor_nodes, freqs, amplitude=0.5, periods=20.0, samples_per_period=4096):
        ...
        for i, v_inst in enumerate(v_in_array):
            if i * dt < 0.25:
                ...
                n_v = {r.name: 0.0 for r in self.resistors}
                n_i = {j.name: 0.0 for j in self.jfets}
...
class CircuitAnalyzer:
    ...
    def run_transient(self):
        f_base = self.freqs[0]
        target_simulation_sec = 0.44
        integer_periods = int(np.round(target_simulation_sec * f_base))
    ...
    def report_multi_frequency_thd(self, node="OUT"):
        print(f"\n--- Origin-Sorted Multi-Frequency THD Tracker ({node} - Steady State) ---")
        start_idx = int(0.40 / self.circuit.dt)
        v_out_steady = self.v_out_data[node][start_idx:]
        v_out_ac = v_out_steady - np.mean(v_out_steady)
        N = len(v_out_ac)
    ...
    def plot_waveforms(self, mode):
        ...
        t_plot_ms = (t_sliced - 0.40) * 1000.0
        ...
        v_out_sliced = self.v_out_data["OUT"][start_idx:]
        v_out_ac = v_out_sliced - np.mean(v_out_sliced)
        N = len(v_out_ac)
        Y = np.fft.rfft(v_out_ac * np.hanning(N))
        xf = np.fft.rfftfreq(N, d=self.circuit.dt)
        mag = 2.0/N * np.abs(Y)
        ...
        ax_fft.set_title("Origin-Sorted Harmonic Spectrum (OUT - Steady State)", fontsize=12)
...
def process_unified_circuit(sim, d, comp_list):
    saved_nodes = {}
    for comp in comp_list:
        ctype = comp.get("type")
        name = comp.get("name")
        if sim is not None and ctype in ["Resistor", "Capacitor", "Inductor", "JFET"]:
        ...
        if d is not None and "draw" in comp:
            draw = comp["draw"]
            el_type = draw.get("elem")
            if el_type == "Dot": el = elm.Dot()
            elif el_type == "Line": el = elm.Line()
            elif el_type == "Ground": el = elm.Ground()
            elif el_type == "Vdd": el = elm.Vdd()
            elif el_type == "Resistor": el = elm.Resistor()
            elif el_type == "Capacitor": el = elm.Capacitor()
            elif el_type == "Inductor": el = elm.Inductor()
            elif el_type == "JFetN": el = elm.JFetN()
            else: continue
            
            if draw.get("dir") == "right": el.right()
            elif draw.get("dir") == "left": el.left()
            elif draw.get("dir") == "up": el.up()
            elif draw.get("dir") == "down": el.down()
            
            if "length" in draw: el.length(draw["length"])
            if "label" in draw: el.label(draw["label"])
            if draw.get("flip"): el.flip()
            if "anchor" in draw: el.anchor(draw["anchor"])
            if "at" in draw: el.at(saved_nodes.get(draw["at"], draw["at"]))
            
            added_el = d.add(el)
            if "save_here" in draw: saved_nodes[draw["save_here"]] = d.here
            if "save_source" in draw: saved_nodes[draw["save_source"]] = added_el.source
            if "save_drain" in draw: saved_nodes[draw["save_drain"]] = added_el.drain
...
if __name__ == "__main__":
    ...
    for mode in ["Clean", "OD1", "OD2"]:
        ...
        rs1_val_num = 2200.0 if mode in ["Clean", "OD1"] else 470.0
        rs1_val_str = "2.2kΩ" if mode in ["Clean", "OD1"] else "470Ω"
        
        core_list = [
            {"type": "Visual", "draw": {"elem": "Dot", "label": "v_ideal", "save_here": "node_v_ideal"}},
            {"type": "Inductor", "name": "L_PICKUP", "val": 4.5, "n1": "v_ideal", "n2": "IN", "r_dc": 8000.0, "c_p": 150e-12, "draw": {"elem": "Inductor", "dir": "right", "label": "L_PICKUP\n4.5H", "save_here": "node_in"}},
            {"type": "Visual", "draw": {"elem": "Dot", "label": "IN", "at": "node_in"}},
            {"type": "Capacitor", "name": "C_IN", "val": 22e-9, "n1": "IN", "n2": "G1", "draw": {"elem": "Capacitor", "dir": "right", "label": "C_IN\n22nF", "save_here": "node_g1"}},
            {"type": "Resistor", "name": "R_G1", "val": 1.0e6, "n1": "G1", "n2": "-", "draw": {"elem": "Resistor", "dir": "down", "label": "R_G1\n1MΩ"}},
            {"type": "Visual", "draw": {"elem": "Ground"}},
            {"type": "JFET", "name": "J1", "idss": 0.0055, "vp": -2.5, "nd": "D1", "ng": "G1", "ns": "S1", "draw": {"elem": "JFetN", "flip": True, "anchor": "gate", "label": "J1\nLSK489", "at": "node_g1", "save_source": "j1_s", "save_drain": "j1_d"}},
            {"type": "Resistor", "name": "R_S1", "val": rs1_val_num, "n1": "S1", "n2": "-", "draw": {"elem": "Resistor", "dir": "down", "label": f"R_S1\n{rs1_val_str}", "at": "j1_s"}},
            {"type": "Visual", "draw": {"elem": "Ground"}}
        ]
        
        if mode in ["OD1", "OD2"]:
            core_list.extend([
                {"type": "Visual", "draw": {"elem": "Line", "dir": "right", "length": 1.5, "at": "j1_s"}},
                {"type": "Capacitor", "name": "C_S1", "val": 22e-6, "n1": "S1", "n2": "-", "draw": {"elem": "Capacitor", "dir": "down", "label": "C_S1\n22μF"}},
                {"type": "Visual", "draw": {"elem": "Ground"}}
            ])
            
        core_list.extend([
            {"type": "Visual", "draw": {"elem": "Line", "dir": "up", "length": 0.5, "at": "j1_d", "save_here": "node_d1"}},
            {"type": "Visual", "draw": {"elem": "Dot", "label": "D1", "at": "node_d1"}},
            {"type": "Resistor", "name": "R_D1", "val": 10000.0, "n1": "+", "n2": "D1", "draw": {"elem": "Resistor", "dir": "up", "label": "R_D1\n10kΩ"}},
            {"type": "Visual", "draw": {"elem": "Vdd", "label": "+18V"}},
            {"type": "Visual", "draw": {"elem": "Line", "dir": "right", "length": 1.5, "at": "node_d1"}},
            {"type": "Capacitor", "name": "C_MID", "val": 22e-9, "n1": "D1", "n2": "G2", "draw": {"elem": "Capacitor", "dir": "right", "label": "C_MID\n22nF", "save_here": "node_g2"}},
            {"type": "Resistor", "name": "R_G2", "val": 1.0e6, "n1": "G2", "n2": "-", "draw": {"elem": "Resistor", "dir": "down", "label": "R_G2\n1MΩ"}},
            {"type": "Visual", "draw": {"elem": "Ground"}},
            {"type": "JFET", "name": "J2", "idss": 0.0055, "vp": -2.5, "nd": "D2", "ng": "G2", "ns": "S2", "draw": {"elem": "JFetN", "flip": True, "anchor": "gate", "label": "J2\nLSK489", "at": "node_g2", "save_source": "j2_s", "save_drain": "j2_d"}},
            {"type": "Resistor", "name": "R_S2", "val": 2200.0, "n1": "S2", "n2": "-", "draw": {"elem": "Resistor", "dir": "down", "label": "R_S2\n2.2kΩ", "at": "j2_s"}},
            {"type": "Visual", "draw": {"elem": "Ground"}},
            {"type": "Visual", "draw": {"elem": "Line", "dir": "up", "length": 0.5, "at": "j2_d", "save_here": "node_d2"}},
            {"type": "Resistor", "name": "R_D2", "val": 10000.0, "n1": "+", "n2": "D2", "draw": {"elem": "Resistor", "dir": "up", "label": "R_D2\n10kΩ"}},
            {"type": "Visual", "draw": {"elem": "Vdd", "label": "+18V"}},
            
            {"type": "Capacitor", "name": "C_OUT", "val": 22e-9, "n1": "D2", "n2": "OUT", "draw": {"elem": "Capacitor", "dir": "right", "label": "C_OUT\n22nF", "at": "node_d2", "save_here": "node_out"}},
            {"type": "Resistor", "name": "R_LOAD", "val": 100000.0, "n1": "OUT", "n2": "-", "draw": {"elem": "Resistor", "dir": "down", "label": "R_LOAD\n100kΩ", "at": "node_out"}},
            {"type": "Visual", "draw": {"elem": "Ground"}},
            {"type": "Visual", "draw": {"elem": "Dot", "label": "OUT", "at": "node_out"}}
        ])

        sim = Circuit(v_dd_ideal=18.0, r_psu=100.0)
        process_unified_circuit(sim, None, core_list)

        analyzer = CircuitAnalyzer(circuit=sim, monitor_nodes=monitor_nodes, input_node="v_ideal", amplitude=0.25)
        
        target_audio_seconds = 2.0  # Modify this global variable to 2.0, 3.0, etc.
        ...
        analyzer.export_audio(mode=mode, target_duration_sec=target_audio_seconds)
        
        with schemdraw.Drawing(file=f"schematic_{mode}.png", show=False) as d:
            d.config(fontsize=10)
            process_unified_circuit(None, d, core_list)
